    ...
    if m.text == "المطورين":
        if sudo2(m):
            try:
                ...
                if lang is None:
                    await m.reply_text(f"◍ [𝐌𝐀𝐑𝐂𝐄𝐋𝐎](tg://user?id={super_sudoers[0]})\n" +
                                       f"◍ [{n.first_name}](tg://user?id={sudoers[0]})\n\n"
                                       "لا يوجد مطورين مرفوعين\n√",
                                       parse_mode=enums.ParseMode.MARKDOWN)
                else:
                    t = "\n◍ قائمة المطورين \n≪━━━━━━━━━━━━━≫\n" + f"◍ [𝐌𝐀𝐑𝐂𝐄𝐋𝐎](tg://user?id={super_sudoers[0]})\n" + \
                        f"◍ [{n.first_name}](tg://user?id={sudoers[0]})\n\n√"
                    for row in lang:
                        t = t + f"[{row[1]}](tg://user?id={row[0]})\n"
                    await m.reply_text(t, parse_mode=enums.ParseMode.MARKDOWN)
...
########################################################################################################################
    ...
    if m.text == (get_db_botname() or "ثيو"):
        texting = ["اؤمر " + (get_db_botname() or "ثيو") + " شتريد؟❤️🥺",
                   "اى يقلب " + (get_db_botname() or "ثيو") + "❤️",
                   ...
                   "قلبى بينادى على " + (get_db_botname() or "ثيو") + "😘",
                   "نعسان محدش يصحينى🙄"
                   ]
        await m.reply_text(random.choice(texting))
        return

    if m.text == "بوت غادر" or m.text == (get_db_botname() or "ثيو") + " غادر":
        if secsudo(m):
            try:
...
########################################################################################################################
    ...
    if m.text == "السورس" or m.text == "سورس" or m.text == "يا سورس":
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [ InlineKeyboardButton ( "CH SOURCE", url=f"https://t.me/Source_Theo" )], 
        [InlineKeyboardButton("اضف البوت الي مجموعتك ✅",
                              url=f"https://t.me/{get_bot_information () [1]}?startgroup=new")],
        ])
        await m.reply_text("""
╭──── • ◈ • ────╮
么 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗵𝗲𝗼](t.me/Source_Theo)
么 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗕𝗼𝘁](t.me/R40_bot)
么 [𝗠𝗮𝗿𝗰𝗲𝗹𝗼](t.me/XIX_A)
╰──── • ◈ • ────╯
...
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
    ...
    if m.text == "مارسيلو" or m.text == "مرسليو":
        s = await c.get_users("6367618993")
        ss = (await c.get_chat("6367618993")).bio
        sender_name = f"[{m.from_user.first_name}](tg://user?id={m.from_user.id})"
        name_chat = m.chat.title
        id_chat = m.chat.id
        if m.chat.type == enums.ChatType.SUPERGROUP:
        ...
        try:
            await c.get_chat_photos("6367618993", limit=1).__anext__()
        except:
            await m.reply_text(f"{ss}", reply_markup=keyboard),
            await c.send_message("6367618993", f"❤️╖ نداء لك ايها المبرمج\n📟╢ بواسطة {sender_name}\n📆╢ يوم *{today}*\n🕑╢ الساعه *{clock}*\n💌╢ اسم الجروب {name_chat}\n🔰╢ ايدي الجروب *{id_chat}*\n⚙️╢ عدد اعضاء الجروب *{num_member}*\n⛓╢ رابط المسج {message_link}\n🔍╜ الرابط {link_group}", parse_mode=enums.ParseMode.MARKDOWN)
        else:
            async for photo in c.get_chat_photos("6367618993", limit=1):
